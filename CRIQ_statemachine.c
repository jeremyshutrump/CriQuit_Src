...
void Statemachine_stateAction(StateUnion *su){
	...
	switch (su->bytes.State){
		...
		case MAXIS_1_RDY:
			// Change ADC channels
			CRIQ_maxisChange();
			//locate sound source and put in temporary variable
			ang = CRIQ_locate();
		//update nextstate for case of negative ID
		su->bytes.NextState = ALERT;
		//if positive ID made, move to Attack and update angle of incidence
			if(CRIQ_ID()){
				a = 0x11;
